...
namespace MongoDB.Context.Mapping
{
	public class ModelMap<T> where T : IMongoDbDocument
  {
    public List<ModelMapRegistry<T>> Registry { get; private set; } = new List<ModelMapRegistry<T>>();
		...
		public void ToCollectionName(string name) => Registry.Add(new ModelMapRegistry<T>
		{
			Type = "CollectionName",
			Name = name
		});

		public void HasObjectId(Expression<Func<T, object>> property) => Registry.Add(new ModelMapRegistry<T>
		{
			Type = "ID",
			Property = property,
			Name = "_id",
		...
		public void Property(Expression<Func<T, object>> property, string name, bool isRequired = false) => Registry.Add(new ModelMapRegistry<T>
		{
			Type = "Property",
			Property = property,
			Name = name,
		...
		internal void CompileModel() => BsonClassMap.RegisterClassMap<T>(t =>
		{
