	...
	public class MongoInMemoryCollection<T> : IMongoContextCollection<T> where T : IMongoDbDocument
  {
    ...
    public void SeedData(IEnumerable<T> data)
    {
      if (data == null)
		...
		public async Task<T> AddAsync(T item, InsertOneOptions opts = null) => await Task.Run(() =>
		{
		...
		public Task<T> FindAsync(string id) => Task.Run(() =>
		{
			try
			{
		...
		public async Task<T> UpdateAsync(string id, T item, ReplaceOptions opts = null) => await Task.Run(async () =>
		{
		...
		public async Task<bool> RemoveAsync(string id) => await Task.Run(() =>
		{
		...
		public async Task<T> FirstOrDefaultAsync(Expression<Func<T, bool>> expr) => await Task.Run(() =>
		{
